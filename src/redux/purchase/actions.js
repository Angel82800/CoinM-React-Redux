...
let PurchaseActions = {
    ...
    createPayment: function (data, token) {
        return (dispatch) => {
            ...
            return axios.post("payments", qs.stringify(data))
                .then((response) => {
    ...
    tokenBuys: function (data, token) {
        return (dispatch) => {
            ...
            return axios.post("token-buys", qs.stringify(data))
                .then((response) => {
    ...
    createWallet: function (data, token) {
        return (dispatch) => {
            dispatch(this.createWalletStart());

            setBearerToken(token);

            return axios.post("users/wallets", qs.stringify(data))
                .then((response) => {
                    return dispatch(this.createWalletSuccess(response.data.data));
                })
                .catch((error) => {
                    if (error && error.response) {
                        dispatch(this.createWalletError(error.response.data.response.errors.message));
                    } else {
                        dispatch(this.createWalletError(error && error.message));
                    }
                })
        }
    },
    ...
    getBalances: function (token) {
        return (dispatch) => {
          dispatch(this.getBalancesStart());
    
          setBearerToken(token);
    
          return axios.get('users/balances')
            .then(response =>
              dispatch(this.getBalancesSuccess(response.data.data))
            )
            .catch((error) => {
              AccountActions.acceptErrorResponse(error, dispatch, token);
              if (error.response) {
                return dispatch(this.getBalancesError(error.response.data.response.errors.message));
              } else {
                return dispatch(this.getBalancesError(error.message));
              }
            })
        }
      },
    
      getBalancesStart: function () {
        return {
          type: "GET_BALANCES_START"
        }
      },
    
      getBalancesSuccess: function (data) {
        return {
      ...
      getBalancesError: function (errorMsg) {
        return {
    ...
    getWallets: function (token) {
        return (dispatch) => {
            dispatch(this.getWalletsStart());

            setBearerToken(token);

            return axios.get("users/wallets")
                .then((response) => {
    ...
    getTokenBuys: function (userId, paymentId, token) {
        return (dispatch) => {
            ...
            return axios.get("token-buys?paymentId=" + paymentId)
                .then((response) => {
