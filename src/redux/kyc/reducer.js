...
function kycReducer(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case actionTypes.UPLOAD_DOCUMENT_SUCCESS: {
            return Object.assign({}, state, {
                existingDataID: (action.docType === "ID" && action.data.extractedData) ? {
                    firstName: action.data.extractedData.Forename || state.existingDataID.firstName,
                    lastName: action.data.extractedData.Surname || state.existingDataID.lastName,
                    DOB: action.data.extractedData.DOB || state.existingDataID.DOB,
                    nationality: action.data.extractedData.Nationality || state.existingDataID.nationality,
                    gender: action.data.extractedData.Gender || state.existingDataID.gender,
        ...
        case actionTypes.VERIFICATION_CHECKING_SUCCESS: {
            if (action.data &&
                ...
                if (action.data.ID.outcome === "OK" && action.data.POA.outcome === "OK")
                    status = 'success';
                else if (action.data.ID.outcome !== "FAIL" && action.data.POA.outcome !== "FAIL")
                    status = 'pending';
                else
                    status = "failure";
