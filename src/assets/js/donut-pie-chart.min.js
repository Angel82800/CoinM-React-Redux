!function(a){
	...
	b.prototype.display=function(){
		...
		this.svg=d3.select(this.$self[0]).append("svg").attr("width",2*b).attr("height",2*b).append("g"),this.svg.append("g").attr("class","slices"),this.pie=d3.layout.pie().sort(null).value(function(a){return a.hvalue}),this.arc=d3.svg.arc().outerRadius(.8*b).innerRadius(.6*b),this.outerArc=d3.svg.arc().innerRadius(.9*b).outerRadius(.9*b),this.svg.attr("transform","translate("+b+","+b+")");
